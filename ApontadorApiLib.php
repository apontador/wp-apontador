...
*/
function apontadorRedirectAutorizacao($config=array ()) {
if ($config['key'])
	extract($config);
else
...
$resposta=file_get_contents($req_req);
parse_str($resposta);
...
*/
function apontadorProcessaAutorizacao($config=array()) {
if ($config)
	extract($config);
else
	extract(apontadorGetConfig());
...
*/
function apontadorChamaApi($verbo="GET", $metodo, $params=array(), $oauth_token="", $oauth_token_secret="",$config=array()) {
  if ($config)
	extract($config);
  else
	extract(apontadorGetConfig());
